...
- `Locales/` — `enUS.lua` makes `ns.L`, keyed by the English phrase; the packager fills `Translations.lua` from
  CurseForge at release. Player-visible text is a whole `L["..."]` phrase (format strings included); after changing
  one, `python3 tools/phrases.py > Locales/phrases.txt`. `tests/locale_spec.lua` checks both.
