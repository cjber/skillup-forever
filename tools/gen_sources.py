...
from gen_trainer import CLASSICDB_CACHE, CLASSICDB_COMMIT, LEARN_SPELL, TEACH_BUILD, classicdb, teach_effects
...
def generate(ids, tables, effect_rows, factions, maps):
    teaches = defaultdict(set)
    for row in effect_rows:
        if int(row["DifficultyID"]) == 0 and int(row["Effect"]) == LEARN_SPELL:
    ...
    used_titles = {q: titles[q] for s in sources.values() for q in s["quests"]}
    used_npcs = {e for s in sources.values() for e in s["vendors"] + [d for d, _ in s["drops"]]}
    return sources, {e: npcs[e] for e in used_npcs}, used_titles, instances
...
def render(sources, npcs, titles, instances):
    lines = [
    ...
    lines += [f"\t[{q}] = {lua_string(t)}," for q, t in sorted(titles.items())]
    lines += ["}", "", "-- [world map] = dungeon or raid name, for drops the zone map can't show", "-- stylua: ignore",
              "ns.InstanceNames = {"]
    used = {n[2] for n in npcs.values()}
    lines += [f"\t[{m}] = {lua_string(name)}," for m, name in sorted(instances.items()) if m in used]
    lines.append("}")
    return "\n".join(lines) + "\n"
...
def main():
    ...
    factions = {int(r["ID"]): r for r in db2("FactionTemplate", ("ID", "FactionGroup", "FriendGroup", "EnemyGroup"), **options)}
    sources, npcs, titles, instances = generate(
        threshold_ids(), dump_tables(**options), teach_effects(**options), factions, db2("Map", ("ID", "MapName_lang", "InstanceType"), **options),
    )
    OUTPUT.write_text(render(sources, npcs, titles, instances), encoding="utf-8")
    print(f"Wrote {OUTPUT.relative_to(ROOT)}: {len(sources)} recipes, {len(npcs)} npcs")
